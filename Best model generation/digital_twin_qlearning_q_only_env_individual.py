...
Usage:
  python digital_twin_qlearning_q_only_env_individual.py --users 50 --days 180 --start-date 2024-01-01 --out-data data/sim_dataset.csv
"""
...
# ---------------------------
# Individual Environment (per user exposure)
# ---------------------------

@dataclass
class Exposure:
    air_quality_index: int = 55
    noise_db: float = 58.0
    light_pollution_lux: float = 8.0
    heatwave: int = 0
    flood: int = 0

def init_exposures(n: int) -> List[Exposure]:
    exps = []
    for _ in range(n):
        exps.append(Exposure(
            air_quality_index=int(np.clip(np.random.normal(55, 10), 0, 300)),
            noise_db=float(np.clip(np.random.normal(58, 5), 30, 100)),
            light_pollution_lux=float(np.clip(np.random.normal(8, 2), 0, 50)),
            heatwave=0, flood=0
        ))
    return exps

def update_exposure(exp: Exposure):
    exp.air_quality_index = int(np.clip(exp.air_quality_index + np.random.normal(0, 8), 0, 300))
    exp.noise_db = float(np.clip(exp.noise_db + np.random.normal(0, 4), 30, 100))
    exp.light_pollution_lux = float(np.clip(exp.light_pollution_lux + np.random.normal(0, 1.5), 0, 50))
    exp.heatwave = 1 if np.random.rand() < 0.02 else 0
    exp.flood = 1 if np.random.rand() < 0.005 else 0

def action_effects(user: User, action: str, exp: Exposure) -> Dict[str, float]:
    ...
    if not adhere:
        ...
        delta["eco"] += abs(noise(0.2))
        return delta

    bad_air = exp.air_quality_index > WHO_ENV["air_quality_max"]
    bad_noise = exp.noise_db > WHO_ENV["noise_max"]
    bad_light = exp.light_pollution_lux > WHO_ENV["light_max"]
    extreme = exp.heatwave or exp.flood

    if action == "sleep_early":
        delta["sleep"] += 1.5 + noise(0.5)
        delta["stress"] += -0.4 + noise(0.3)
        delta["screen"] += -0.3 + noise(0.2)
        if bad_light: delta["sleep"] += -0.3; delta["eco"] += 0.1
    elif action == "walk_30":
        delta["activity"] += 30 + int(np.random.normal(0, 8))
        if not (bad_air or extreme):
            delta["stress"] += -0.5 + noise(0.25); delta["eco"] += -0.3; delta["sleep"] += 0.2 + noise(0.2)
        else:
            delta["stress"] += 0.1 + noise(0.2); delta["eco"] += 0.25
        delta["social"] += 2 + int(np.random.normal(0, 2))
    elif action == "eat_veg_400":
        delta["diet"] += 300 + int(np.random.normal(0, 80)); delta["stress"] += -0.1 + noise(0.2)
    elif action == "no_tobacco_alcohol":
        delta["habit"] += 1; delta["stress"] += -0.2 + noise(0.3); delta["eco"] += -0.05
    elif action == "meditate_10":
        delta["stress"] += -0.9 + noise(0.3); delta["sleep"] += 0.2 + noise(0.2); delta["mindful"] += 10; delta["eco"] += -0.4
    elif action == "drink_water_8":
        delta["water"] += 4 + int(np.random.normal(0, 2))
    elif action == "digital_detox_30":
        delta["screen"] += -0.5 + noise(0.2); delta["sleep"] += 0.1 + noise(0.2); delta["stress"] += -0.1 + noise(0.2); delta["eco"] += -0.1
    elif action == "social_15":
        delta["social"] += 15 + int(np.random.normal(0, 4)); delta["stress"] += -0.25 + noise(0.2); delta["eco"] += -0.15
    return delta

def apply_daily_drift(user: User, exp: Exposure):
    ...
    eco_drift = 0.0
    if exp.air_quality_index > WHO_ENV["air_quality_max"]: eco_drift += 0.15
    if exp.noise_db > WHO_ENV["noise_max"]: eco_drift += 0.1
    if exp.light_pollution_lux > WHO_ENV["light_max"]: eco_drift += 0.05
    if exp.heatwave: eco_drift += 0.3; stress_drift += 0.2
    if exp.flood: eco_drift += 0.5; stress_drift += 0.3
...
def who_compliance(u: User, exp: Exposure) -> Dict[str, int]:
    return {
        ...
        "air_quality": int(exp.air_quality_index <= WHO_ENV["air_quality_max"]),
        "noise": int(exp.noise_db <= WHO_ENV["noise_max"]),
        "light": int(exp.light_pollution_lux <= WHO_ENV["light_max"]),
        "heatwave_safe": int(exp.heatwave == 0),
        "flood_safe": int(exp.flood == 0)
    }

def reward_and_penalty(u: User, action: str, exp: Exposure) -> Tuple[float, float, Dict[str,int]]:
    comp = who_compliance(u, exp)
    reward = (
    ...
    deficits = {
        ...
        "Air": max(0, exp.air_quality_index - WHO_ENV["air_quality_max"]),
        "Noise": max(0.0, exp.noise_db - WHO_ENV["noise_max"]),
        "Light": max(0.0, exp.light_pollution_lux - WHO_ENV["light_max"]),
    }
    ...
    # small shaping re-use
    lam = 1.0
    edge_reward = 10.0 - lam * (1.0 if dominant_from != dominant_to else 0.0)
    edge_reward = max(-10.0, min(10.0, edge_reward))
...
class DigitalTwinSim:
    def __init__(self, cfg: SimulationConfig):
        ...
        self.logs: List[Dict] = []
        self.exposures: List[Exposure] = init_exposures(cfg.users)

    def step_user(self, u: User, exp: Exposure, action: str, day: int, date: pd.Timestamp):
        delta = action_effects(u, action, exp)
        ...
        clip_user_state(u)
        shaped_reward, penalty, comp = reward_and_penalty(u, action, exp)
        observed = misreport(shaped_reward - penalty, scale=0.15)
        apply_daily_drift(u, exp)
        clip_user_state(u)
        ...
        self.logs.append({
            ...
            # personal exposure snapshot
            "air_quality_index": exp.air_quality_index,
            "noise_db": exp.noise_db,
            "light_pollution_lux": exp.light_pollution_lux,
            "heatwave": exp.heatwave,
            "flood": exp.flood,
            # compliance flags
            **comp
        })
    ...
    def simulate(self, start_date: str = "2023-01-01") -> float:
        ...
        for day, date in enumerate(dates, start=1):
            for i, u in enumerate(self.population):
                update_exposure(self.exposures[i])
                exp = self.exposures[i]
                state = encode_state(who_compliance(u, exp))
                a_idx = self.agent.select_action(state)
                r, pen, comp, obs = self.step_user(u, exp, ACTIONS[a_idx], day, date)
                r_eff = r - pen
                next_state = encode_state(who_compliance(u, exp))
                self.agent.update(state, a_idx, r_eff, next_state)
                cum[u.user_id] += r_eff
        return float(cum.mean())

    def save_logs(self, out_path: str = "data/sim_dataset_env_individual.csv", save_parquet: bool = False):
...
def save_best_model(best_Q: np.ndarray, meta: Dict, outdir="models", filename="best_q_agent_env_individual.pkl"):
    out_dir = Path(outdir)
    out_dir.mkdir(parents=True, exist_ok=True)
    payload = {"ACTIONS": ACTIONS, "Q": best_Q, "n_states": 16, "n_actions": len(ACTIONS), "meta": meta}
    with open(out_dir / filename, "wb") as f:
        pickle.dump(payload, f)
    with open(out_dir / "best_q_meta_env_individual.json", "w") as f:
        json.dump(meta, f, indent=2)
    print(f"\nSaved best Q-learning model → {out_dir / filename}")
    print(f"Saved metadata              → {out_dir / 'best_q_meta_env_individual.json'}")

def main():
    parser = argparse.ArgumentParser(description="Digital Twin Q-learning with individual environmental exposures")
    ...
    parser.add_argument("--model-filename", type=str, default="best_q_agent_env_individual.pkl")
    parser.add_argument("--start-date", type=str, default="2023-01-01")
    parser.add_argument("--out-data", type=str, default="data/sim_dataset_env_individual.csv")
