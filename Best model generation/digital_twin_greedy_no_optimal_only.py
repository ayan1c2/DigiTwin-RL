...
- Single-run greedy policy with configurable samples_per_action (Monte Carlo lookahead)
- Saves the greedy policy config to pickle
...
# ---------------------------
# Model Save Helper (no grid search)
# ---------------------------

def save_greedy_model(best_samples: int, stats: Dict[str, np.ndarray], outdir="models", filename="greedy_policy.pkl"):
    ...
    with open(Path(outdir) / "greedy_policy_meta.json", "w") as f:
        json.dump({
...
Saved greedy policy → {Path(outdir) / filename}")
    print(f"Saved metadata      → {Path(outdir) / 'greedy_policy_meta.json'}")
...
if __name__ == "__main__":
    DAYS = 365
    USERS = 25
    SAMPLES_PER_ACTION = 5  # tweak if desired

    # Re-seed for reproducibility
    random.seed(SEED); np.random.seed(SEED)

    cfg = SimulationConfig(days=DAYS, users=USERS, samples_per_action=SAMPLES_PER_ACTION)
    sim = DigitalTwinGreedySim(cfg)

    print("Running greedy simulation...")
    mean_cum_reward = sim.simulate()

    stats = sim.agent.get_stats()
    save_greedy_model(cfg.samples_per_action, stats, outdir="models", filename="greedy_policy.pkl")

    print("
=== Summary ===")
    print(f"Days: {DAYS}, Users: {USERS}, samples_per_action: {SAMPLES_PER_ACTION}")
    print(f"Mean cumulative effective reward per user: {mean_cum_reward:.3f}")
    print("Done.")
