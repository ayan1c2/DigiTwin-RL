...
- NEW: Persist full per-day dataset (CSV/optional Parquet) with proper ISO dates
- NEW: Customizable users/days via CLI args

Usage:
  python digital_twin_qlearning_q_only_with_dataset.py --users 50 --days 180 --start-date 2024-01-01 --out-data data/sim_dataset.csv
"""
...
class DigitalTwinSim:
    ...
    def step_user(self, u: User, action: str, day: int, date: pd.Timestamp):
        ...
        self.logs.append({
            ...
            "penalty": penalty,
            "observed": observed,
            **comp  # expands compliance flags into columns: sleep, activity, diet, habit, water
        })
...
def main():
    parser = argparse.ArgumentParser(description="Digital Twin Q-learning with dataset logging")
